...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        with torch.no_grad():
            ...
            # Initialize a column vector filled with end-of-sentence(eos) tokens. 
            # Shape[0] is the batch_size or the number of test data in our case, shape[1] is the length namely one token.
            go_slice = \
            ...
            # We keep one more candidate as a backoff, when encourtering the unkonwn(unk) token.
        ...
        for _ in range(args.max_len-1):
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    # We can keep track of how many paths are finished with add_final. 
                    # Othrewise, we will keep searching after encountering eos.
                    ...
                    if next_word[-1] == tgt_dict.eos_idx:
                        ...
                        search.add_final(-node.eval(args.alpha) + gama*j, node)
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        search.add(-node.eval(args.alpha) + gama*j, node)

            # #import pdb;pdb.set_trace()
            # __QUESTION 5: What happens internally when we prune our beams?
            # How do we know we always maintain the best sequences?
            # We remove all nodes but the beam_size best ones. Only the top nodes are kept.
            for search in searches:
                search.prune()

        # Segment into sentences
        # Stack the n best list 
        best_n_lists = []
        n = 3
        for search in searches:
            best_n_nodes = search.get_n_best(n)
            for node in best_n_nodes:
                best_n_lists.append(node[1].sequence[1:].cpu())
            
        best_sents = torch.stack([best_n_list for best_n_list in best_n_lists])
        ...
        # We crop the sentences with multiple eos, and keep only the part before the first eos.
        temp = list()
        ...
        # every n best list is stored as a list in the according place

        for ii, sent in enumerate(output_sentences):
            if ii % n == 0:
                all_hyps[int(sample['id'].data[count])] = [sent]
            else:
                all_hyps[int(sample['id'].data[count])].append(sent)
                if ii % n == n-1:
                    count += 1
    ...
    if args.output is not None:
        with open(args.output, 'w') as out_file:
            for sent_id in range(len(all_hyps.keys())):
                # read out each sentence in the n best list
